...
fn map_video_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<VideoItem> {
  ...
  Ok(VideoItem {
    ...
    offset_ms: row.get(13)?,
    created_at: ts_to_dt(row.get(14)?),
    updated_at: ts_to_dt(row.get(15)?),
    deleted_at: row.get::<_, Option<i64>>(16)?.map(ts_to_dt),
  })
}
...
impl VideoRepository for SqliteVideoRepository {
      ...
      tx.execute(
        "INSERT OR IGNORE INTO video_review_item
         (batch_id, file_path, status, thumbnail_path, duration_ms, fps, width, height,
          video_codec, audio_codec, bitrate_kbps, offset_ms, created_at, updated_at)
         VALUES (?1, ?2, 'pending', ?3, ?4, ?5, ?6, ?7, ?8, ?9, ?10, 0, ?11, ?11)",
        params![
          batch_id,
  ...
  fn list_videos(&self, batch_id: i64, filter: &VideoFilter) -> VideoReviewResult<Vec<VideoItem>> {
    let mut sql = String::from(
      "SELECT id, batch_id, file_path, status, remark, thumbnail_path, duration_ms, fps, width, height,
              video_codec, audio_codec, bitrate_kbps, offset_ms, created_at, updated_at, deleted_at
  ...
  fn add_marker(
    ...
    self.conn.execute(
      "INSERT INTO video_review_marker (video_id, time_ms, kind, text, severity, created_at)
       VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
      params![video_id, time_ms as i64, kind.to_sql(), text, severity, now_ts()],
    )?;
    Ok(self.conn.last_insert_rowid())
  }
  ...
  fn delete_marker(&self, id: i64) -> VideoReviewResult<()> {
    self.conn.execute("DELETE FROM video_review_marker WHERE id = ?1", [id])?;
    Ok(())
  }
  ...
  fn delete_segment(&self, id: i64) -> VideoReviewResult<()> {
    self.conn.execute("DELETE FROM video_review_segment WHERE id = ?1", [id])?;
    Ok(())
  }
...
mod tests {
  ...
  fn crud_batch_and_video() {
    ...
    repo
      .update_video_status(videos[0].id, ReviewStatus::Approved)
      .unwrap();
  ...
  fn markers_and_segments() {
    ...
    repo
      .add_marker(vid, 1000, MarkerKind::Issue, "抖动", 2)
      .unwrap();
    repo
      .add_segment(vid, 0, 5000, "片头", ReviewStatus::NeedsFix)
      .unwrap();
