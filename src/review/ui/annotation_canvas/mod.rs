...
pub use types::{
    ...
    AnnotationType, CanvasTool, NormRect, NormalizedRect,
};

use std::collections::hash_map::DefaultHasher;
use std::hash::{Hash, Hasher};

use eframe::egui::{self, Color32, Context, Key, Pos2, Rect, Sense, TextureHandle, Ui, Vec2};
...
impl AnnotationCanvas {
    ...
    pub fn focus_on_annotation(&mut self, ann: &Annotation) {
        crate::review::domain::coords::center_viewport_on_norm(
            &mut self.transform,
            ann.focus_norm(),
        );
    ...
    pub fn ui_with_options(
        ...
        if let Some(fp) = &self.pending_select {
            if let Some(ann) = annotations.iter().find(|a| fp.matches(a)) {
                ...
                events.push(Event::SelectionChanged { id: Some(ann.id) });
            }
        }
        ...
        let pan_mode = options.allow_pan_zoom
            && (ui.input(|i| {
                i.pointer.middle_down() || (i.key_down(Key::Space) && i.pointer.primary_down())
        ...
        match self.tool {
            CanvasTool::Select => {
                if let Some(id) = hit_test(annotations, &self.transform, pos) {
                    ...
                    if let Some(ann) = annotations.iter().find(|a| a.id == id) {
                        let grab_norm =
                            screen_to_norm(ScreenPoint { x: pos.x, y: pos.y }, &self.transform);
            ...
            DragState::Creating { tool, start } => {
                if let Some(end) = self.draft_pointer.take() {
                    ...
                    if (norm.x1 - norm.x0).abs() > 0.005 || (norm.y1 - norm.y0).abs() > 0.005 {
                        ...
                        self.pending_select =
                            Some(AnnotationFingerprint::from_parts(kind, &position));
                    }
                }
            }
            DragState::Moving {
            ...
            | DragState::MovingArrowEnd {
                ...
                if let Some((pid, after)) = self.preview.take() {
                    if pid == id && after != start_position {
                        ...
                        events.push(Event::UpdateAnnotation {
                            id,
                            position: after,
                        });
                    }
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    if ui.button("确认").clicked() && !buffer.trim().is_empty() {
                        ...
                        self.pending_select = Some(AnnotationFingerprint::from_parts(
                            AnnotationKind::Text,
                            &position,
                        ));
