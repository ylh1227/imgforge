...
pub fn properties_panel_ui(
    ...
    let Some(item) = item else {
        ui.label(RichText::new("选择图片以查看属性").color(theme::secondary_label(dark)));
        return false;
    };
    ...
    if let Some(meta) = &state.metadata {
        ...
        if meta.exif_fields.is_empty() {
            ui.label(RichText::new("EXIF：未检测到").color(theme::secondary_label(dark)));
        } else {
            ui.collapsing("EXIF 详情", |ui| {
                for field in &meta.exif_fields {
    ...
    ui.horizontal(|ui| {
        ...
        egui::ComboBox::from_id_salt("review_convert_format")
            ...
            .show_ui(ui, |ui| {
                if ui
                    .selectable_label(state.convert_draft.format.is_none(), "默认")
                    .clicked()
                {
