...
impl ContactSheetService {
  pub fn export<P: FrameProvider>(provider: &P, req: &ContactSheetRequest) -> VideoReviewResult<ContactSheetResult> {
    if req.videos.len() < 2 {
...
fn render_cell(
  ...
  let time_label = format!("{} / {}", ms_to_timestamp(effective_ms), ms_to_timestamp(video.duration_ms));
  draw_label(
    &mut cell,
    6,
