...
use crate::review::domain::annotation::{Annotation, AnnotationPosition};
...
use crate::review::storage::sqlite_mappers::{
    color_to_hex, map_annotation_row, map_batch_row, map_image_row, map_tag_row, now_ts,
};
...
impl SqliteReviewRepository {
    ...
    pub fn approved_paths_in_batch(&self, batch_id: i64) -> ReviewResult<Vec<PathBuf>> {
       ...
       WHERE batch_id = ?1 AND status = ?2 ORDER BY file_path",
        ...
        let rows = stmt.query_map(params![batch_id, ReviewStatus::Approved.to_sql()], |row| {
    ...
    pub fn create_tag(&self, name: &str, color: [u8; 4]) -> ReviewResult<i64> {
       ...
       ON CONFLICT(name) DO UPDATE SET color = excluded.color",
        ...
        let id: i64 =
            self.conn
                .query_row("SELECT id FROM review_tag WHERE name = ?1", [name], |row| {
                    row.get(0)
                })?;
        Ok(id)
    }
...
impl ReviewRepository for SqliteReviewRepository {
    ...
    fn delete_batch(&self, id: i64) -> Result<(), ReviewError> {
        ...
        let image_ids: Vec<i64> = {
            let mut stmt = tx.prepare("SELECT id FROM review_image_item WHERE batch_id = ?1")?;
    ...
    fn delete_annotation(&self, annotation_id: i64) -> Result<(), ReviewError> {
        let n = self.conn.execute(
            "DELETE FROM review_annotation WHERE id = ?1",
            [annotation_id],
        )?;
        if n == 0 {
            return Err(ReviewError::NotFound {
