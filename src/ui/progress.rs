...
pub trait ProgressReporter: Send + Sync {
    ...
    fn set_current_label(&self, _label: &str) {}
...
impl GuiProgress {
    ...
    pub fn set_current_label(&self, label: &str) {
        *self.current_file.lock().unwrap() = label.to_string();
    }
...
impl ProgressReporter for GuiProgress {
    ...
    fn set_current_label(&self, label: &str) {
        GuiProgress::set_current_label(self, label);
    }
    ...
    fn status_label(&self) -> Option<String> {
        ...
        if current.is_empty() {
            ...
            Some(format!("正在处理 {done} / {total} · {current}"))
        }
    }
}
