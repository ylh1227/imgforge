...
impl ImgforgeApp {
    ...
    fn retry_convert_failures(&mut self, paths: Vec<PathBuf>) {
                    ...
                    .show(ui, |ui| {
                        ui.vertical_centered(|ui| {
                                ...
                                widgets::grouped_section(ui, "评审队列", |ui| {
                                    ...
                                    ui.horizontal(|ui| {
                                        ui.add_enabled(
                                            enabled,
                                            egui::Checkbox::new(
                                                &mut self.burn_review_annotations,
                                                "导出时叠加标注",
                                            ),
                                        );
                                        if widgets::secondary_button(ui, "清空评审队列", enabled)
                                            .clicked()
                                        {
                            ...
                            widgets::grouped_section(ui, "预设与历史", |ui| {
                                ui.horizontal_wrapped(|ui| {
                                    if !self.gui_prefs.presets.is_empty() {
                                        ...
                                        if widgets::compact_secondary_button(ui, "套用", enabled)
                                            .clicked()
                                        {
                                            if let Some(p) =
                                                self.gui_prefs.presets.get(selected).cloned()
                                            {
                                        ...
                                        if widgets::compact_secondary_button(ui, "删除", enabled)
                                            .clicked()
                                        {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    .clicked()
                                    {
                                        ...
                                        if name.is_empty() {
                                            ...
                                            self.gui_prefs.upsert_preset(
                                                name.clone(),
                                                self.snapshot_from_ui(),
                                            );
                                            let _ = self.gui_prefs.save();
                                ...
                                if !self.gui_prefs.history.is_empty() {
                                    ...
                                    ui.add_space(4.0);
                                    let recent: Vec<_> =
                                        self.gui_prefs.history.iter().take(5).cloned().collect();
                            ...
                            widgets::grouped_section(ui, "转换前摘要", |ui| {
                                ui.horizontal(|ui| {
                                    if widgets::compact_secondary_button(ui, "刷新预估", enabled)
                                        .clicked()
                                    {
                                        self.refresh_previews();
                                    }
                                ...
                                if let Some(ref preview) = self.batch_preview {
                                    for line in preview
                                        .summary_lines(self.formats[self.format_index].extension())
                                    {
