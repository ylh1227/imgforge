...
use super::annotation::{
  Annotation, AnnotationKind, AnnotationPosition, RectanglePosition,
};
  ...
  for ann in annotations {
    ...
    match (&ann.kind, &ann.position) {
      ...
      (AnnotationKind::Text, AnnotationPosition::Text(t)) => {
        ...
        let marker = Rect::at(p.x, p.y).of_size((text.len() as u32 * 6).min(120).max(12), 14);
...
fn draw_arrow_head(img: &mut RgbaImage, from: super::coords::PixelPoint, to: super::coords::PixelPoint, color: Rgba<u8>) {
  ...
  let uy = dy / len;
  let size = 8.0;
  let left = (to.x as f32 - ux * size - uy * size * 0.5, to.y as f32 - uy * size + ux * size * 0.5);
  let right = (to.x as f32 - ux * size + uy * size * 0.5, to.y as f32 - uy * size - ux * size * 0.5);
