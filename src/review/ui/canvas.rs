...
use eframe::egui::{
    self, Color32, Context, Painter, Pos2, Rect, Sense, Stroke, TextureHandle, Ui, Vec2,
};
...
impl CanvasState {
    pub fn new() -> Self {
        Self {
            ...
            transform: ViewportTransform::fit_image(
                egui_bridge::vec2(Vec2::new(400.0, 300.0)),
                (1, 1),
            ),
            image_size: (1, 1),
            annotations: Vec::new(),
    ...
    pub fn actual_size(&mut self, canvas: Vec2) {
        self.transform = ViewportTransform::one_to_one(egui_bridge::vec2(canvas), self.image_size);
                    ...
                    DrawTool::Text => {
                        ...
                        if !self.text_input.trim().is_empty() {
                            return Some(Annotation::new_draft(
                                0,
                                AnnotationKind::Text,
                                AnnotationPosition::Text(TextPosition {
                                    x: norm.x,
                                    y: norm.y,
                                }),
                                self.style.clone(),
