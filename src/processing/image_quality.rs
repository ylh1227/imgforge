...
pub fn apply_unsharp_mask(image: &mut DynamicImage, radius: f32, amount: f32, threshold: u8) {
    if amount <= 0.0 {
        return;
    }
    ...
    for ((orig, blur), dst) in rgba.pixels().zip(blurred.pixels()).zip(out.pixels_mut()) {
        for channel in 0..3 {
...
fn resize_linear_light(
    ...
    let src_image =
        FirImage::from_vec_u8(src_w, src_h, src_bytes, fir::PixelType::F32x4).map_err(|e| {
            AppError::Pipeline {
                step: "resize".into(),
                reason: e.to_string(),
            }
...
fn pick_resize_alg(scale: f32) -> fir::ResizeOptions {
    if scale > 1.0 {
        ...
        fir::ResizeOptions::new().resize_alg(fir::ResizeAlg::Convolution(fir::FilterType::Mitchell))
    } else if scale < 0.5 {
        // 强缩小：超采样 + Lanczos3（接近 PS「双三次 sharper / 保留细节」）
        fir::ResizeOptions::new()
            .resize_alg(fir::ResizeAlg::SuperSampling(fir::FilterType::Lanczos3, 2))
    } else {
...
mod tests {
    ...
    fn fill_mode_center_crops_to_target() {
        ...
        let out = resize_image(&source, Some(100), Some(100), ResizeMode::Fill).unwrap();
