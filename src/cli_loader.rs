...
pub fn load_config(cli: &Cli) -> AppResult<AppConfig> {
  ...
  if let Some(ref preset_name) = cli.preset {
    let preset = Preset::parse(preset_name).ok_or_else(|| {
      AppError::Config(format!("unknown preset: {preset_name}"))
    })?;
...
fn apply_cli_overrides(config: &mut AppConfig, cli: &Cli) {
  ...
  if cli.watermark_image.is_some()
    || cli.watermark_text.is_some()
    || cli.watermark_font.is_some()
  {
  ...
  if let Some(ref sizes) = cli.thumbnail_sizes {
    ...
    for spec in sizes {
      match imgforge::core::types::parse_thumbnail_spec(spec) {
        ...
        Err(e) => tracing::warn!(spec = %spec, error = %e, "skipping invalid thumbnail spec"),
      }
    }
  }
