...
impl CompareView {
    ...
    let detail = match self.mode {
      ...
      CompareDisplayMode::MultiSplit => {
        if self.multi_pane_count <= 2 {
          format!("{} · {} 张 · 左右并排", self.mode.label(), self.multi_pane_count)
        } else {
          format!(
            "{} · {} 张 · 宫格布局",
            self.mode.label(),
  ...
  fn fit_multi_panes(
    &mut self,
    canvas_size: Vec2,
    apply: impl Fn(&mut AnnotationCanvas, Vec2),
  ) {
  ...
  pub fn ui(
    ...
    if textures_pending {
      ctx.request_repaint_after(std::time::Duration::from_millis(if self.defer_texture_load {
        80
      } else {
        32
      }));
  ...
  pub fn ui_multi(
    ...
    if missing > 0 {
      ui.label(
        RichText::new(format!("已显示 {} 张，其余 {missing} 张加载中…", loaded.len()))
          .size(12.0)
