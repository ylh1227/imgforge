...
use super::types::{Annotation, AnnotationFingerprint, AnnotationKind, AnnotationPosition, AnnotationStyle};
...
impl HistoryStack {
  ...
  pub fn record_update(&mut self, id: i64, before: AnnotationPosition, after: AnnotationPosition) {
  ...
  pub fn record_content(&mut self, id: i64, before: String, after: String) {
    self.undo.push(HistoryEntry::UpdateContent { id, before, after });
    self.redo.clear();
  }
  ...
  pub fn redo(&mut self, _annotations: &[Annotation]) -> Option<AnnotationCanvasEvent> {
    ...
    let event = match &entry {
      ...
      HistoryEntry::UpdateContent { id, after, .. } => AnnotationCanvasEvent::UpdateAnnotationContent {
        id: *id,
        content: after.clone(),
      },
    };
