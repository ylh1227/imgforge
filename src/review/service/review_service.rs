...
impl ReviewService {
    ...
    ) -> ReviewResult<BatchStatusResult> {
    ...
    pub fn batch_add_remarks(
        &self,
        request: &BatchRemarkRequest,
    ) -> ReviewResult<BatchRemarkResult> {
    ...
    pub fn create_batch_from_folder(
        ...
        self.batch_service()
            .create_from_folder(name, folder, recursive)
    }
...
impl ReviewConversionBridge for ReviewService {
    ...
    fn burn_annotations_for_export(
        ...
        let img = image::open(output).map_err(|e| ReviewError::ImageDecode {
...
impl ReviewService {
    ...
    pub fn refresh_metadata_cache(&self, id: i64, path: &Path) -> ReviewResult<ImageMetadata> {
        ...
        self.repo
            .update_image_metadata(id, meta.file_size, meta.width, meta.height)?;
        Ok(meta)
    }
