...
impl IncrementalProcessor {
  ...
  pub fn save(&self) -> AppResult<()> {
    ...
    {
      let content =
        toml::to_string_pretty(&self.state).map_err(|e| AppError::Incremental(e.to_string()))?;
      if let Some(parent) = self.state_path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| AppError::Incremental(e.to_string()))?;
      }
...
fn compute_file_hash(path: &Path) -> AppResult<String> {
  use std::io::Read;
  let mut file =
    std::fs::File::open(path).map_err(|e| AppError::Incremental(e.to_string()))?;
