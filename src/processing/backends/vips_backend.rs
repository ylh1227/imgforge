...
mod vips_impl {
    ...
    pub fn decode(ctx: &mut ImageContext) -> AppResult<()> {
        ...
        let vips_img =
            VipsImage::new_from_buffer(bytes, "").map_err(|e| AppError::DecodeFailed {
        ...
        let image = match bands {
            ...
            other => {
                return Err(decode_err(ctx, &format!("unsupported band count: {other}")));
            }
        };
    ...
    pub fn encode(ctx: &mut ImageContext) -> AppResult<()> {
        ...
        let vips_img = VipsImage::new_from_memory(
            &raw,
            w as i32,
            h as i32,
            4,
            rs_vips::enums::BandFormat::Uchar,
        )
